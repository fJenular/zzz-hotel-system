...
export default function ReceptionistCheckOutPage() {
  ...
  const handleCheckOut = async (bookingId: string, roomId: string) => {
    try {
      ...
      alert('Check-Out completed successfully! Cleaning task generated.')
      fetchBookings()
    } catch (err: any) {
    ...
    <div className="flex min-h-screen bg-gray-50/50 font-sans text-gray-800 antialiased">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-100 p-6 shrink-0 justify-between">
        <div className="space-y-8">
          ...
          <nav className="space-y-1">
            ...
            <Link href="/receptionist/check-in" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-900 rounded-xl transition-all duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Check-Out Operations</h1>
            <p className="text-xs text-gray-500">Manage departing guests, settle folios, and mark rooms for cleaning.</p>
          </div>
          <button 
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          {loading ? (
            <div className="text-center py-16 text-gray-500 font-semibold animate-pulse">Loading check-out queue...</div>
          ) : bookings.length === 0 ? (
            <div className="bg-white border border-gray-100 p-12 text-center rounded-2xl">
              ...
              <p className="text-xs text-gray-400 mt-1">There are no checked-in guests awaiting check-out at this time.</p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-400 uppercase">Room Occupied</th>
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-400 uppercase">Stay Period</th>
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-400 uppercase">Checkout Due</th>
                    ...
                    <tr key={booking.id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          ...
                          className="px-3.5 py-1.5 text-xs font-bold bg-rose-500 hover:bg-rose-600 text-white rounded-xl transition shadow-sm"
                        >
                          Complete Check-Out
                        </button>
                      </td>
